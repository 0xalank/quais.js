  ...
  return {
    request(method, params, signal) {
      if (disposed)
        return Promise.reject(
          new WalletConnectorError("DISCONNECTED", "Connector disposed"),
        );
      if (pending)
        return Promise.reject(
      ...
      if (signal?.aborted)
        return Promise.reject(
          new WalletConnectorError("CANCELLED", "Request cancelled"),
        );
      try {
        parseParams(method, params);
      } catch {
        return Promise.reject(
          new WalletConnectorError("INVALID_REQUEST", "Invalid wallet request"),
        );
      }
      ...
      if (!popup)
        return Promise.reject(
          new WalletConnectorError(
            "POPUP_BLOCKED",
            "Allow the wallet popup and try again.",
          ),
        );
      const peer = popup,
      ...
      if (!boundedMessage(payload))
        return Promise.reject(
          new WalletConnectorError("INVALID_REQUEST", "Request too large"),
        );
      peer.focus();
      return new Promise((resolve, reject) => {
        ...
        const receive = (event: ConnectorMessageEvent) => {
          ...
          if (
            data?.protocol !== CONNECTOR_PROTOCOL ||
            data.version !== 1 ||
    ...
    destroy() {
      ...
      pending?.fail(
        new WalletConnectorError(
          "DISCONNECTED",
          "Connector disposed. Submitted operations remain on-chain.",
        ),
      );
      popup?.close();
      popup = null;
